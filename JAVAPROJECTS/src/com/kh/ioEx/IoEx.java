...
1. java.io 패키지(폴더)
    ...
    BufferedReader / BufferedWriter : 효율적으로 한 줄 씩 문자 입출력 처리
2. java.nio 패키지(폴더)
...
Stream(스트림)
    ...
    Buffer 로 모여진 데이터를 출력해서 보여주는 역할
//
//Buffer(버퍼)
//    입출력 성능을 향상시키기 위해 데이터를 일시적으로 저장하는 공간
//    버퍼링 중 : 데이터를 모으는 중
...
- 작업이 완료되면 알림을 통해 결과를 전달받음 *알림 : callback event
- 호출한 쪽은 작업이 끝날 때 까지 기다리지 않아도 되므로 병렬 처리 가능
