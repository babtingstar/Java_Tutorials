...
- setCharAt(int index, char ch) : 지정한 위치의문자를 다른 문자 하나로 변경
- subString(int start, int end) : 지정된 범위의 문자열 반환
- subString(int start) : 지정된 위치부터 문자열의 끝까지 반환
...
public class StringBuilderEx {
    ...
    public void methodStringBuilder() {
        ...
        // sb2에 World 문자열 추가
        sb2.append("World");
        System.out.println("World 라는 문자가 추가되었는지 확인 : " + sb2);

        // insert 를 이용해서 6번째 위치에 Java 글자 삽입, index는 0부터 시작
        sb2.insert(5, " Java ");
        System.out.println("Java 라는 문자가 추가되었는지 확인 : " + sb2);

        // replace 를 이용해서 6 ~ 10 범위를 Python 으로 대체
        sb2.replace(6, 10, "Python");
        System.out.println("6 - 10 을 Python 으로 교체 : " + sb2);

        // 6 ~ 13 범위의 값을 삭제
        sb2.delete(6, 13);
        System.out.println("6 - 13 을 삭제 : " + sb2);

        // reverse 를 이용해서 sb2 문자열을 반전
        sb2.reverse();
        System.out.println("reverse 사용 : " + sb2);
    }
}
